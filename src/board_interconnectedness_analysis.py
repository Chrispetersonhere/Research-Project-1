...
The script estimates whether board interconnectedness (directors sitting on other S&P 500 boards)
is associated with future stock performance.
"""
...
def pull_sp500_membership(conn: wrds.Connection, start_year: int, end_year: int) -> pd.DataFrame:
    """Pull S&P 500 constituent-month panel and aggregate to firm-year membership."""
    query = f"""
        SELECT
            m.permno,
            m.start,
    ...
    ccm["linkenddt"] = ccm["linkenddt"].fillna(pd.Timestamp("today"))
...
def pull_director_memberships(
    ...
    """Pull director-firm-year data from a WRDS directors table.

    Defaults target `risk.directors` with common column names. Use CLI args if your WRDS
    table has different names.
    """
    query = f"""
        SELECT
            CAST({director_gvkey_col} AS TEXT) AS gvkey,
            CAST({director_id_col} AS TEXT) AS director_id,
            CAST({director_year_col} AS INT) AS year
        FROM {director_table}
        WHERE {director_year_col} BETWEEN {start_year} AND {end_year}
          AND {director_gvkey_col} IS NOT NULL
          AND {director_id_col} IS NOT NULL
    """
...
def compute_interlock_metrics(sp500_firm_year: pd.DataFrame, director_panel: pd.DataFrame) -> pd.DataFrame:
    ...
    seats = (
        board.groupby(["year", "director_id"]) ["gvkey"]
        .nunique()
        .rename("n_sp500_boards")
        .reset_index()
    )
...
def save_outputs(panel: pd.DataFrame, model_df: pd.DataFrame, model, output_dir: Path) -> None:
    ...
    with open(output_dir / "board_interlock_regression.txt", "w", encoding="utf-8") as f:
        f.write("S&P 500 BOARD INTERLOCKNESS AND FUTURE STOCK PERFORMANCE\n")
